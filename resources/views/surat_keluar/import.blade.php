...
<div class="mb-8">
    <div class="flex items-start gap-4">
        <a href="{{ route('surat-keluar.index') }}" class="mt-1 inline-flex h-10 w-10 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-500 shadow-sm hover:bg-slate-50 hover:text-slate-700">
        ...
        <div>
            ...
            <h1 class="mt-1 text-2xl font-bold text-slate-950">Import Surat Keluar</h1>
            <p class="mt-1 max-w-2xl text-sm text-slate-500">Upload workbook agenda lama dari kantor. Sheet <code>Surat Keluar</code> akan dibaca otomatis.</p>
        </div>
    </div>
</div>
...
<div class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
    <form action="{{ route('surat-keluar.import.store') }}" method="POST" enctype="multipart/form-data" data-turbo="false" data-loading-form data-loading-message="Membaca file Excel dan menyiapkan preview..." class="self-start overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-slate-200">
        @csrf
        ...
        <div class="p-6">
            <label for="file" class="group flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-300 bg-slate-50/70 px-6 py-10 text-center transition hover:border-indigo-300 hover:bg-indigo-50/60">
                ...
                <span>
                    <span class="mt-4 block text-sm font-semibold text-slate-900">Klik untuk memilih file agenda surat keluar</span>
                    <span class="mt-1 block text-sm text-slate-500">Mendukung format kantor lama dengan cell merge.</span>
                </span>
            </label>
    ...
    <aside class="self-start rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
        ...
        <div class="mt-4 space-y-3 text-sm">
            <div class="flex gap-3">
                ...
                <p class="text-slate-600">Sheet <code>Surat Keluar</code>, data baris 7 kolom B-H.</p>
            </div>
            <div class="flex gap-3">
                <span class="mt-1 h-2 w-2 rounded-full bg-indigo-500"></span>
                <p class="text-slate-600">Tanggal keluar yang digabung diteruskan otomatis ke baris bawah.</p>
            </div>
            <div class="flex gap-3">
                <span class="mt-1 h-2 w-2 rounded-full bg-indigo-500"></span>
        ...
        <div class="mt-5 rounded-lg bg-slate-50 p-4 ring-1 ring-slate-100">
            <p class="text-xs font-bold uppercase tracking-wide text-slate-500">Kolom</p>
            <p class="mt-2 text-sm text-slate-600">Tanggal keluar, nomor urut, penerima, tanggal surat, nomor, perihal, dan asal.</p>
        </div>
    </aside>
</div>
@endunless
            ...
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div class="flex flex-col gap-3 xl:flex-row xl:items-center">
                    ...
                    <div class="flex flex-col gap-2 sm:flex-row">
                        <a href="{{ route('surat-keluar.import') }}" class="inline-flex justify-center rounded-md bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 ring-1 ring-slate-300 hover:bg-slate-50">Upload Ulang</a>
                        <form action="{{ route('surat-keluar.import.store') }}" method="POST" data-turbo="false" data-loading-form data-loading-message="Menyimpan data valid ke database...">
                            @csrf
            ...
            <table class="min-w-full divide-y divide-slate-200 text-sm">
                <thead class="sticky top-0 z-10 bg-slate-50 shadow-sm">
                    <tr>
                        ...
                        <th class="px-3 py-3 text-left font-semibold text-slate-700">Tgl Keluar</th>
                        <th class="px-3 py-3 text-left font-semibold text-slate-700">No Urut</th>
                        <th class="px-3 py-3 text-left font-semibold text-slate-700">Penerima</th>
                        ...
                        <th class="px-3 py-3 text-left font-semibold text-slate-700">Asal</th>
                ...
                <tbody class="divide-y divide-slate-100 bg-white">
                    @forelse($previewRows as $row)
                        ...
                        <tr class="{{ $row['status'] === 'error' ? 'bg-red-50/30' : ($row['status'] === 'duplicate' ? 'bg-amber-50/30' : '') }}">
                            ...
                            <td class="whitespace-nowrap px-3 py-3 text-slate-700">{{ $row['data']['tanggal_keluar_surat'] ?? '-' }}</td>
                            <td class="whitespace-nowrap px-3 py-3 text-slate-700">{{ $row['data']['nomor_urut'] ?? '-' }}</td>
                            <td class="min-w-48 px-3 py-3 text-slate-700">{{ $row['data']['alamat_penerima'] ?? '-' }}</td>
                            ...
                            <td class="whitespace-nowrap px-3 py-3 text-slate-700">{{ $row['data']['asal'] ?? '-' }}</td>
...
@if(session('import_errors'))
    <div class="mt-6 rounded-xl bg-white p-5 shadow-sm ring-1 ring-amber-200">
