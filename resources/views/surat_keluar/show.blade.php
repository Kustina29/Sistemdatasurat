    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <div class="flex items-center gap-2">
                <h1 class="text-lg sm:text-2xl font-bold text-slate-900 tracking-tight">Detail Surat Keluar</h1>
            ...
            <p class="mt-0.5 text-xs sm:text-sm text-slate-500 hidden sm:block">Tinjau informasi lengkap dan lampiran dokumen surat keluar.</p>
        </div>
    </div>
    <a href="{{ route('surat-keluar.edit', $surat_keluar->id) }}" class="shrink-0 inline-flex items-center gap-1.5 rounded-xl bg-gradient-to-r from-indigo-500 to-indigo-600 px-3 py-2 sm:px-4 sm:py-2.5 text-sm font-semibold text-white shadow-md shadow-indigo-500/20 hover:from-indigo-600 hover:to-indigo-700 transition-all">
    ...
    <section class="rounded-xl sm:rounded-2xl bg-white p-4 sm:p-6 lg:p-8 shadow-sm ring-1 ring-slate-100 lg:col-span-2">
        ...
        <div class="grid gap-x-8 gap-y-6 sm:grid-cols-2">
            ...
            <div class="rounded-xl bg-slate-50/50 p-4 ring-1 ring-slate-100 transition-colors hover:bg-slate-50">
                <p class="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-1">Tanggal Keluar</p>
                <div class="flex items-center gap-2">
                    ...
                    <p class="text-sm font-bold text-slate-800">{{ \Carbon\Carbon::parse($surat_keluar->tanggal_keluar_surat)->format('d F Y') }}</p>
                </div>
            </div>
            ...
            <div class="rounded-xl bg-slate-50/50 p-4 ring-1 ring-slate-100 transition-colors hover:bg-slate-50">
                ...
                <div class="flex items-center gap-2">
                    <svg class="h-4 w-4 text-indigo-400" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5" /></svg>
                    <p class="text-sm font-bold text-slate-800">{{ \Carbon\Carbon::parse($surat_keluar->tanggal_surat)->format('d F Y') }}</p>
                </div>
            </div>

            <!-- Nomor Surat -->
            <div class="rounded-xl bg-slate-50/50 p-4 ring-1 ring-slate-100 transition-colors hover:bg-slate-50 sm:col-span-2">
                <p class="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-1">Nomor Surat</p>
                <div class="flex items-center gap-2">
                    ...
                    <p class="text-sm font-bold text-slate-800">{{ $surat_keluar->nomor_surat ?: '-' }}</p>
                </div>
            </div>

            <!-- Penerima -->
            <div class="rounded-xl bg-slate-50/50 p-4 ring-1 ring-slate-100 transition-colors hover:bg-slate-50 sm:col-span-2">
                <p class="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-1">Instansi / Penerima</p>
                <div class="flex items-start gap-2">
                    ...
                    <p class="text-sm font-semibold text-slate-800">{{ $surat_keluar->alamat_penerima ?: '-' }}</p>
                </div>
            </div>

            <!-- Asal -->
            <div class="rounded-xl bg-slate-50/50 p-4 ring-1 ring-slate-100 transition-colors hover:bg-slate-50 sm:col-span-2">
                <p class="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-1">Asal / Pengirim Internal</p>
                <div class="flex items-start gap-2">
                    ...
                    <p class="text-sm font-semibold text-slate-800">{{ $surat_keluar->asal ?: '-' }}</p>
                </div>
            </div>
            ...
            <div class="rounded-xl bg-indigo-50/30 p-5 ring-1 ring-indigo-100 sm:col-span-2">
                ...
                <p class="whitespace-pre-line text-sm font-medium text-slate-800 leading-relaxed">{{ $surat_keluar->perihal }}</p>
            </div>
        </div>
    </section>
        ...
        @if($surat_keluar->lampiran_path)
            <div class="group relative rounded-xl border border-slate-200 bg-slate-50 p-5 hover:bg-slate-100 transition-all duration-300">
                <div class="flex flex-col items-center text-center">
                    ...
                    <p class="break-words text-sm font-bold text-slate-800 line-clamp-2" title="{{ $surat_keluar->lampiran_nama_asli }}">
                        {{ $surat_keluar->lampiran_nama_asli }}
                    </p>
                    <p class="mt-2 inline-flex items-center rounded-full bg-slate-200/60 px-2.5 py-0.5 text-xs font-semibold text-slate-600">
                        {{ $surat_keluar->lampiran_ukuran_label ?: 'Ukuran tidak diketahui' }}
                    </p>
                </div>
                
                <a href="{{ $surat_keluar->lampiran_url }}" target="_blank" class="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-slate-800 px-4 py-3 text-sm font-semibold text-white hover:bg-slate-700 transition-colors shadow-sm">
